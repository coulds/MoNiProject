...
public class Kawaii_LoadingView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        // 控件资源名称
        TypedArray typedArray = context.obtainStyledAttributes(attrs, com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView);

        // 方块行数量(最少3行)
        lineNumber = typedArray.getInteger(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_lineNumber, 3);
        if (lineNumber < 3) {
            lineNumber = 3;
        }

        // 半个方块的宽度（dp）
        half_BlockWidth = typedArray.getDimension(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_half_BlockWidth, 30);
        // 方块间隔宽度（dp）
        blockInterval = typedArray.getDimension(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_blockInterval, 10);

        // 移动方块的圆角半径
        moveBlock_Angle = typedArray.getFloat(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_moveBlock_Angle, 10);
        // 固定方块的圆角半径
        fixBlock_Angle = typedArray.getFloat(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_fixBlock_Angle, 30);
        // 通过设置两个方块的圆角半径使得二者不同可以得到更好的动画效果哦

        // 方块颜色（使用十六进制代码，如#333、#8e8e8e）
        int defaultColor = context.getResources().getColor(com.edu.fragmentlayout.R.color.colorAccent); // 默认颜色
        blockColor = typedArray.getColor(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_blockColor, defaultColor);

        // 移动方块的初始位置（即空白位置）
        initPosition = typedArray.getInteger(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_initPosition, 0);
        ...
        // 动画方向是否 = 顺时针旋转
        isClock_Wise = typedArray.getBoolean(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_isClock_Wise, true);

        // 移动方块的移动速度
        // 注：不建议使用者将速度调得过快
        // 因为会导致ValueAnimator动画对象频繁重复的创建，存在内存抖动
        moveSpeed = typedArray.getInteger(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_moveSpeed, 250);

        // 设置移动方块动画的插值器
        int move_InterpolatorResId = typedArray.getResourceId(com.edu.fragmentlayout.R.styleable.Kawaii_LoadingView_move_Interpolator,
